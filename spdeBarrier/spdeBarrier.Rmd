...
We recomend to go trough [the case study with standard SPDE-procedure](https://github.com/skaug/tmb-case-studies/tree/barrier/spde) first. Here we use an extension of that procedure were the spatial range is decreased by including barriers on land.



```{r,echo=FALSE, comment="",fig.cap="Figure: Mesh set up in R-INLA,  used for the GMRF representation in TMB. Blue polygons illustrate coastline and boundary, and red points illustrate observations and observation size. Grey area illustrates where the spatial range is decreased.", out.width = "500px"}
...
```
* The vector `barrier.triangles` consists of indecies to all triangles which have center point on land. The triangles are shown in the figure above. Inside these triangles we decrease the spatial range.
</details>
