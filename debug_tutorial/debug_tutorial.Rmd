...
* Linux: debugging works without problems, but it is recommended to
not use RStudio when doing debugging (use the ordinary R command line prompt instead).
* Mac: ?
* Windows: The behaviour of `gdb` has varied across
  versions of R and Rtools. In case of problems, please consult: [https://github.com/kaskr/adcomp/wiki/Windows-installation#windows-debugging](https://github.com/kaskr/adcomp/wiki/Windows-installation#windows-debugging). Further, allthough it is possible to run the simple example in this tutorial under Windows, larger .cpp files will not work. It is generally recommended to install [Windows Subsystem for Linux](https://en.wikipedia.org/wiki/Windows_Subsystem_for_Linux). It is a bit of work setting it up, but once installed it is easy to switch over to that whenever it is necessary to use the debugger.
...
(gdb)
```
This prints the code line that caused the crash. Usually this
is enough to find the source of the error, but below we will 
explore more advanced debugging features.

**Comment** The appropriate "frame number" (here \#2) will vary from model to model. 
Experiment with different numbers in the range 2-5 until you see the text
 `in objective_function<double>::operator()` (and the name of your .cpp file).
</details>
...
If you are only interested in the line number of the error (does not currently work under Windows):
```r 
...
It is very useful to  be able to see the value stored
