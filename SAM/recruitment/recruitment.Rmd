...
Type objective_function<Type>::operator()(){
  ...
  // See exercise 1 below for negative log-likelihood
...
`recruit1.R` change `recruitment` to `recruit1` in 3 places.
...
Assume that we don't observe the recruitment process directly and that measurement error is introduced. We assume that the measurement error $\eta_y \sim N(0, \sigma_{R^\text{obs}}^2)$ and iid. The model can then be expressed as:

\begin{align}
  \log R_y^{(\text{obs})} &= \log R_y + \eta_y, \\
  \log R_y &= \log R_{y-1} + \epsilon_y. 
\end{align}
...
* Ricker: $f(SSB) = \alpha\cdot SSB \cdot e^{-\beta\cdot SSB}$
* Beverton-Holt: $f(SSB) = \frac{\alpha\cdot SSB}{1 + \beta\cdot SSB}$

We can then set up our recruitment process like: 
...
1. 
<details> 
  <summary> Implement the Beverton-Holt model in the TMB program (note that the parameters are on a log scale) 
  </summary> 
