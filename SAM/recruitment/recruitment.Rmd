...
<details> 
  ...
  \begin{align}
    L(\theta) &= \int f(R^\text{obs}, R) dR = \int f(R^\text{obs} | R) f(R) dR \\
    &= \int f(R_1^\text{obs} | R_1) f(R_1) \prod_{y=2}^T f(R_y^\text{obs} | R_y) f(R_y) dR
  \end{align}
...
Instead of letting the recruitment be a function of the recruitment from the year before, we can use the spawning stock biomass from last year to predict the recruitment. We will look at two popular options: 

* Richer: $RS(SSB) = \alpha SSB e^{-\beta SSB}$
* Beverton-Holt: $RS(SSB) = \frac{\alpha SSB}{1 + \beta SSB}$

We can then setup our recruitment process like: 

\[
  \log R_y = \log RS(SSB_{y - 1}) + \epsilon_y, \quad \text{where } \epsilon \sim N(0, \sigma_R^2)
\]
