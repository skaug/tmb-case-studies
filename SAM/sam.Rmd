...
where $R(\cdot)$ is a function describing the spawning productivity of the cod, $A$ represents a maximum age, $F$ represents the fishing mortality and $M$ the natural mortality. It is assumed that the process error $\eta_{a,y}$ is iid N(0,$\sigma_{\eta,a}^2$), i.e. independently normally distributed. We combine all age specific quantities into vectors: 
...
<details> <summary> Example data: survey and commercial catch data in year 1999.</summary>
...
```
The variable `fleet` provides information about whether the observation is commercial catch or which survey it is from. In this example the value 1 refers to catch data, and 2 and 3 to the two separate surveys.
</details>
...
<details>
  <summary>Calculate the likelihood contribution from process model for fishing mortality $F$ </summary>
```c++
...
<details>
  <summary>Calculate the SSB, used to predict recruitment </summary>
```c++
...
<details> <summary> Calculate the likelihood contribution from the process model for $N$ (equations above).  </summary>
```c++
...
<details>
<summary> Calculate the likelihood contribution from the observations (indexes and commercial landings)   </summary>
```c++
...
<details>
<summary>Calculate the average fishery mortality ($\bar F$), which is returned for illustration </summary>
```c++
...
<details>
<summary>Calculate the estimated catch, which is returned for illustration </summary>
```c++
...
<details>
<summary>Calculate the recruitment, which is returned for illustration </summary>
```c++
  for(int y=0;y<timeSteps;y++){  
    logR(y)=logN(0,y);
    R(y)=exp(logR(y));
