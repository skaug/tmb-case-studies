...
As we did with the recruitment process, we will model the recruitment as a state space model in the following way: 
...
1. 
<details> 
  <summary> Implement the contribution from $N_{a,y}$ for $1 < a < A$. </summary> 
    ```c++
...
The last latent process in SAM is the fishing mortality $F_{a,y}$ for age $a$ in year $y$. As with the survival process, we will model this as a random walk with observational error, but we will also have add a correlation structure in the year dimension. If we let $F_y = (F_{1,y}, \ldots, F_{A,y})$, we can formulate a model for $F_y$ as

\[
  \log F_y = \log F_{y-1} + \psi_y,
\]

where $\psi$ follows an A (maximum age) -dimensional multivariate normal distribution with expectation 0 and covariance matrix $\Sigma$. There are many options for the structure of $\Sigma$, but we will focus on three alternatives: 
...
\[
  \log F_{a,y}^\text{obs} = \log F_{a,y} + \epsilon_{ay}, \quad \epsilon_{ay} \sim N(0, \sigma_{F^\text{obs}}^2)
\]
...
2. 
<details> 
  <summary> Compare the models with different correlation structure and find the best one based on AUC      </summary> 
