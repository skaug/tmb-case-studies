...
\[
  \log N_{ay}^{(\text{obs})} = \log N_{a,y} +  \eta_{ay}, \quad \text{where } \eta_{ay} \sim N(0, \sigma^2).
\]
...
\[
  \log F_y = \log F_{y-1} + \xi_y,
\]

where $\xi_y$ follows an A (maximum age) -dimensional multivariate normal distribution with expectation 0 and covariance matrix $\Sigma$. There are many options for the structure of $\Sigma$, but we will focus on three alternatives: 
